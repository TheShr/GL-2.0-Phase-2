...
export async function GET() {
  try {
    ...
    const telemetryPath = path.join(rootDir, "..", "backend", "output", "telemetry_dump.json");

    if (fs.existsSync(telemetryPath)) {
      try {
        const telemetryRaw = fs.readFileSync(telemetryPath, "utf-8");
        const telemetryData = JSON.parse(telemetryRaw);
        if (telemetryData["0"]) {
          console.log("[API] Serving data from telemetry_dump.json");
          return NextResponse.json(telemetryData["0"]);
        }
      } catch (err: any) {
    ...
    const clustersPath = path.join(rootDir, "..", "backend", "output", "hotspot_clusters.csv");
    const schedulePath = path.join(rootDir, "..", "backend", "output", "enforcement_schedule.csv");

    if (!fs.existsSync(clustersPath) || !fs.existsSync(schedulePath)) {
      return NextResponse.json(
        { error: "ML output files not found. Run recommendations script first." },
        { status: 404 }
      );
    }

    // 1. Parse clusters
    const clustersData = fs.readFileSync(clustersPath, "utf-8");
    ...
    for (let i = 1; i < scheduleLines.length; i++) {
      ...
      // rank,cluster_id,police_station,location_centroid,capacity_reduction_rcf,travel_time_before_min_km,travel_time_after_min_km,delay_savings_per_vehicle_min,total_commuter_time_saved_hours,target_shift,enforcement_action
      // Wait, let's verify if index matches headers
      const row: any = {};
      ...
      // Parse RCF: comes as fraction (0.0–1.0) in the CSV
      const rawRcf = parseFloat(row.capacity_reduction_rcf || "0");
      const cleanRcf = isNaN(rawRcf) ? 0 : rawRcf; // keep as fraction for math; display component will multiply by 100

      const totalHoursSaved = parseFloat((row.total_commuter_time_saved_hours || "0").replace(" hours", "")) || 0;

      // Format travel times: they come as raw float (min/km), format to 1 decimal
    ...
    // capacity_reduction_rcf is a fraction (0-1); multiply by 100 for percentage
